...
def plot_peak_hist(warped_gray, road_width, PIXEL_CONVERSION_RATE, is_plot=True):
    ...
    # warped_gray[:, :int(warped_gray.shape[1] // 2 - (road_width / 2 + 2) / PIXEL_CONVERSION_RATE - 1)] = 1
    # warped_gray[:, int(warped_gray.shape[1] // 2 + (road_width / 2 + 2) / PIXEL_CONVERSION_RATE - 1):] = 1
...
def find_ploy_fit_window_search(binary_warped, leftx_base, rightx_base, following_left_lane, following_right_lane, road_width, PIXEL_CONVERSION_RATE, target_left_pixel_percent, target_right_pixel_percent, full_line_target_percent, roi_limit_L, roi_limit_M, roi_limit_R, PT_view_height, stopx_base, nwindows=9, is_debug=False, margin=100, minpix=50, is_plot=True, add_debug_image=True):
    ...
    # binary_warped[:, int(binary_warped.shape[1] // 2 + (road_width / 2 + 2) / PIXEL_CONVERSION_RATE - 1):] = 0
    binary_warped[: binary_warped.shape[0]//2,:] = 0
...
def plot_detected_poly(out_img, left_fit, right_fit, is_plot=True):
    ...
    for pts, index in enumerate(left_fitx):
        circle(out_img, ((int)(left_fitx[pts]), (int)(ploty[pts])), 3, (255, 255, 255), -1)

    for pts, index in enumerate(right_fitx):
        circle(out_img, ((int)(right_fitx[pts]), (int)(ploty[pts])), 3, (255, 255, 255), -1)
...
def plot_lanes_unwrap(viewsize, left_fit, right_fit, left_turn, right_turn, undistorted_in_rgb, M_INV, stopx_base, roi_limit_M, roi_limit_L, roi_limit_R, originalsize, is_plot=False, add_debug_image=True):
    debug_images = []

    if stopx_base > viewsize[1] / 11:
        stopx_limit = int(stopx_base - viewsize[1] / 11)
    else:
        stopx_limit = 0

    if roi_limit_M > viewsize[1] / 11:
        roi_limit_M = int(roi_limit_M - viewsize[1] / 11)
    else:
        roi_limit_M = 0
    ...
    #     pdb.set_trace()
    left_fitxxx = left_fit[0] * plot_yyy ** 2 + left_fit[1] * plot_yyy + left_fit[2]
    right_fitxxx = right_fit[0] * plot_yyy ** 2 + right_fit[1] * plot_yyy + right_fit[2]
    center_fitxxx = center_fit[0] * plot_yyy ** 2 + center_fit[1] * plot_yyy + center_fit[2]
    left_stop_fitxxx = left_fit[0] * plot_yyy2 ** 2 + left_fit[1] * plot_yyy2 + left_fit[2] + 15
    right_stop_fitxxx = right_fit[0] * plot_yyy2 ** 2 + right_fit[1] * plot_yyy2 + right_fit[2] - 15
...
class image_converter:
    ...
    def lanetracker(self):
        ...
        print('start_lanetracker')
        # rospy.init_node('image_converter', anonymous=True)
        # cam = cv2.VideoCapture('C:/Users/USER/PycharmProjects/line_detect/kcity_last_main_ppt_1_following_left.mp4')
        cam = cv2.VideoCapture(0)

        # cam.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        # cam.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
        # # 960x540, 640x360, 320x180
        ...
        ret, video = cam.read()
        print(video.shape,'-',video.dtype, '-', np.min(video) , '-' , np.max(video))
        ...
        center_fit_deg_ave = 0
        right_fit_deg_ave = 0
        white_tresh_lower_limit = 157
        white_tresh_upper_limit = 205
        reliability = 0
        none_error_loop = 0
        ...
        while True:  # @@@@@@@@@@@@@@@@@@@@영상출력루프 시작@@@@@@@@@@@@@@@@@@@@@@
            ...
            a = 'time_diff = ' + '{:0.4f}'.format(current_time - prev_time) + '   Hz = ' + '{:0.4f}'.format(1 / (current_time - prev_time))
            get_fps = cam.get(cv2.CAP_PROP_FPS)
            print('loop_count : ', loop, '---------------- ', a, '---------------- fps : ', get_fps)
...
# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ1차 로스 추가용 코드ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            lower_yellow = (5, yellow_s_tresh_lower, 80)  # HSV 컬러 영억 검출 임계값 영역 (색상, 채도, 조도)
            ...
            # 확인시 사용 final_line_color = Final_Bynary_Mix(color_brg, binary_detect_morph_brg_half_sum, blue_mask_area_detect_morph, yellow_mask_area_detect_morph)  # 컬러랑 흰색 검출 영역 합치기 #추세선 탐지에 사용한는 영상의 이진화를 하기 전단계
            ...
            # 확인시 사용 final_line_bynary_half_sum = cv2.bitwise_or(color_bynary, binary_detect_morph_brg_half_sum)  # 추세선 탐지에 사용되는 이진화 영상
            ...
            if error != 0:
                ...
                half_sum_left_pix_img = np.copy(binary_detect_morph_brg_half_sum)
                half_sum_left_pix_img[:, int(half_sum_left_pix_img.shape[1] // 2 - road_width / PIXEL_CONVERSION_RATE / 4 - 1):] = 0
                # half_sum_left_pix_img[:, :int(half_sum_left_pix_img.shape[1] // 2 - (road_width / 2 + 1) / PIXEL_CONVERSION_RATE - 1)] = 0
                half_sum_left_pix_img[:half_sum_left_pix_img.shape[0]//2,:] = 0

                half_sum_right_pix_img = np.copy(binary_detect_morph_brg_half_sum)
                half_sum_right_pix_img[:, :int(half_sum_right_pix_img.shape[1] // 2 + road_width / PIXEL_CONVERSION_RATE / 4 - 1)] = 0
                # half_sum_right_pix_img[:, int(half_sum_right_pix_img.shape[1] // 2 + (road_width / 2 + 1) / PIXEL_CONVERSION_RATE - 1):] = 0
                half_sum_right_pix_img[:half_sum_right_pix_img.shape[0]//2, :] = 0

                cv2.imshow('for checking error left lane',half_sum_left_pix_img * 255)
                cv2.imshow('for checking error right lane',half_sum_right_pix_img * 255)
                ...
                bynary_half_sum_left_pixel_percent = np.sum(half_sum_left_pix_img) / (half_sum_left_pix_img.shape[0] * (half_sum_left_pix_img.shape[1] // 2))
                bynary_half_sum_right_pixel_percent = np.sum(half_sum_right_pix_img) / (half_sum_right_pix_img.shape[0] * (half_sum_right_pix_img.shape[1] // 2))
                print('bynary_half_sum_left_pixel_percent : ', bynary_half_sum_left_pixel_percent, '        bynary_half_sum_right_pixel_percent : ',bynary_half_sum_right_pixel_percent )

                if (bynary_half_sum_left_pixel_percent >= full_line_target_percent) & (bynary_half_sum_right_pixel_percent < full_line_target_percent/3*2):
                    following_left_lane = True
                    following_right_lane = False
                elif (bynary_half_sum_right_pixel_percent >= full_line_target_percent) & (bynary_half_sum_left_pixel_percent < full_line_target_percent/3*2):
                    following_right_lane = False
                    following_right_lane = True
                else:
                    following_right_lane = False
                ...
                if white_tresh_lower_half_sum >= white_tresh_upper_limit: white_tresh_lower_half_sum = white_tresh_upper_limit
                if white_tresh_lower_half_sum <= white_tresh_lower_limit: white_tresh_lower_half_sum = white_tresh_lower_limit
            ...
            if error == 0:
                ...
                if error == 0:
                    ...
                    bynary_left_pixel_percent = np.sum(bynary_window_image_left_area) / ((viewsize[1] - roi_limit_L - (roi_limit_bump[0] - roi_limit_bump[1])) * viewsize[0])
                    bynary_right_pixel_percent = np.sum(bynary_window_image_right_area) / ((viewsize[1] - roi_limit_R - (roi_limit_bump[0] - roi_limit_bump[1])) * viewsize[0])
                    ...
                    if white_tresh_lower_left >= white_tresh_upper_limit: white_tresh_lower_left = white_tresh_upper_limit
                    if white_tresh_lower_left <= white_tresh_lower_limit: white_tresh_lower_left = white_tresh_lower_limit
                    ...
                    if white_tresh_lower_right >= white_tresh_upper_limit: white_tresh_lower_right = white_tresh_upper_limit
                    if white_tresh_lower_right <= white_tresh_lower_limit: white_tresh_lower_right = white_tresh_lower_limit
                    ...
                    yellow_lower_limit = 45
                    yellow_upper_limit = 85
                    if yellow_s_tresh_lower >= yellow_upper_limit: yellow_s_tresh_lower = yellow_upper_limit
                    if yellow_s_tresh_lower <= yellow_lower_limit: yellow_s_tresh_lower = yellow_lower_limit
...
# ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ2차 로스 추가용 코드ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            if error == 0:
                ...
                color_stop_warp, newwarp, final_image, debug_images3 = plot_lanes_unwrap(viewsize, left_fit, right_fit, left_turn, right_turn, undistored_image, M_INV, stopx_base, roi_limit_M, roi_limit_L, roi_limit_R, originalsize, is_plot=False,
            ...
            if add_debug_image3:
                ...
                composed_image = cv2.resize(composed_image, dsize=(1280, 720))
                ...
                reliability_text = "reliability : " + "{:}".format(reliability) + " "
                ...
                putText = cv2.putText
                putText(composed_image, left_pix_per_text, (10, 50), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, right_pix_per_text, (10, 90), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, L_target_pixel_percent, (10, 130), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, R_target_pixel_percent, (10, 170), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                if error == 0:
                    putText(composed_image, error_text, (10, 210), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                else:
                    putText(composed_image, error_text, (10, 210), fontFace, fontScale, (0, 0, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, deviation_text, (10, 250), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, loop_text, (10, 290), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, deviation_stop_text, (10, 330), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, length_stop_histogram_text, (10, 370), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, line_type_text, (10, 410), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, max_stop_histogram_text, (10, 450), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)


                putText(composed_image, half_sum_pixel_per_text, (580, 50), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, target_half_sum_pixel_percent_text, (580, 90), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, white_tresh_lower_left_text, (580, 130), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, white_tresh_lower_right_text, (580, 170), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)



                if (reliability >= 3) & (reliability < 7):
                    putText(composed_image, reliability_text, (580, 210), fontFace, fontScale, (200, 0, 255), thickness, lineType=cv2.LINE_AA)
                elif reliability >= 7:
                    putText(composed_image, reliability_text, (580, 210), fontFace, fontScale, (255, 0, 0), thickness, lineType=cv2.LINE_AA)
                else:
                    putText(composed_image, reliability_text, (580, 210), fontFace, fontScale, (0, 0, 255), thickness, lineType=cv2.LINE_AA)


                putText(composed_image, top_offset_text, (580, 250), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, mid_offset_text, (580, 290), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, bot_offset_text, (580, 330), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, tresh_factor_right_text, (580, 370), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, left_fit_deg_ave_text, (580, 410), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, center_fit_deg_ave_text, (580, 450), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, right_fit_deg_ave_text, (580, 490), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
                putText(composed_image, yellow_s_tresh_lower_text, (580, 530), fontFace, fontScale, (100, 200, 255), thickness, lineType=cv2.LINE_AA)
    ...
    #
            if add_debug_image3:
                ...
                composed_image_resized = cv2.resize(composed_image, dsize=(int(round(1280 * imshow_scale_2)), int(round(720 * imshow_scale_2))))
                cv2.imshow('composed_image_resized', composed_image_resized)  # 필

    #
    #             if error == 0:
    #                 base_stop_image_show_resized = cv2.resize(base_stop_image_show, dsize=(int(round(PT_view_width_2 * imshow_scale_1)), int(round(PT_view_height_2 * imshow_scale_1))))
    #                 cv2.imshow('base_stop_image_show_resized', base_stop_image_show_resized*255)
    #
    #
...
target_distance = 15  # 11.25  # 목표 거리 (현재 목표 10m전방)
road_width = 4  # 차선 폭 (현재 3.5미터 실측 후 조정)
margin = 9  # 추가로 투상변환할 폭
...
# proper_size = np.zeros([1280, 1920, 3], dtype=np.uint8)
# proper_size = np.zeros([720, 1280, 3], dtype=np.uint8)
proper_size = np.zeros([480, 640, 3], dtype=np.uint8)
# proper_size = np.zeros([180, 320, 3], dtype=np.uint8)
